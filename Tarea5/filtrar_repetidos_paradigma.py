def repetidos(lista):
    # Utilizamos filter para filtrar los elementos que se repiten más de una vez
    resultado = list(filter(lambda x: lista.count(x) == 1, lista))
    print(resultado)
    return resultado
...
# Llama a la función, cambiar numero de lista para probar resultados
resultado = repetidos(lista2)

##Sin paradigmas (Original)
